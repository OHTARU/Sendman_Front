...
class _PhotoToTextState extends State<PhotoToText> {
  ...
  Future<void> _sendImageToOCR(File image) async {
    const String apiKey = 'AIzaSyAtNXwq7_KtXWDop3ZV_WGryDfmcTTeZaM'; // Replace with your actual API key
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        _extractedText = jsonResponse['responses'][0]['fullTextAnnotation']['text'];
      });
    } else {
      setState(() {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("Text successfully sent to the server"))
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("Failed to send text. Status code: ${response.statusCode}"))
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 20),
            _extractedText != null ? Text(_extractedText!, textAlign: TextAlign.center) : Text('No text extracted yet.'),
            SizedBox(height: 20),
            ElevatedButton(
              onPressed: _extractedText != null ? () => sendTextToServer(_extractedText!) : null,
